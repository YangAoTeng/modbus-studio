...
一个面向工程调试场景的 Modbus RTU / Modbus TCP Client + Server 桌面工具。

![Modbus Studio 界面预览](image/a9a44842-7b27-4651-ab8f-f82f5e49a07d%20(1).png)
...
![寄存器字典视图](image/637e6fa3-892f-444e-9405-574f7d1942a3.png)
...
![Client 视图](image/9916707d-9dee-408d-bb40-cac55858d7bc%20(1).png)
...
![Server 视图](image/dc49edfa-2797-4e6e-b00c-08309d201ec4.png)
...
![工程管理视图](image/3e5a3769-11d2-4914-9940-df8c04db8922.png)
...
通常是旧的打包目录被其他程序占用，例如 Typora、资源管理器预览、杀毒软件或已经启动的 `Modbus Studio.exe`。

处理方法：

1. 关闭正在运行的 `Modbus Studio.exe`。
2. 关闭打开工程目录文件的编辑器或预览窗口。
3. 删除临时目录：

```powershell
Remove-Item .\release\win-unpacked.tmp -Recurse -Force
```
...
1. 在寄存器字典中添加几个 `40001` 开始的保持寄存器点位。
2. 在 Server 页面选择 `Modbus TCP`，监听端口设置为 `1502`。
3. 点击“启动服务”。
4. 在 Client 页面选择 `Modbus TCP`。
5. 主机地址填写 `127.0.0.1`，端口填写 `1502`。
6. 点击“连接 TCP”。
7. 观察 Client 是否按字典自动读取 Server 数据。
8. 修改 Server 中的解析值，观察 Client 下一轮读取是否变化。

### 10.2 真实设备调试

1. 根据设备说明书建立寄存器字典。
2. 确认地址范围、数据类型、长度和比例因子。
3. 连接 RTU 或 TCP 设备。
4. 观察报文日志是否有正常响应。
5. 对可写点位先用小范围安全值测试写入。
6. 保存工程，方便后续继续调试。
...
- 写入真实设备前请确认点位权限、单位和比例因子。
- 对控制类点位建议先在 Server 模拟环境中验证。
- 工程文件会保存报文日志，分享给他人前请确认其中没有敏感设备信息。
